 ...
 */
export function getExamStructure(examName) {
    console.log('[getExamStructure] Lookup for:', examName);
    if (!examName) return null;

    // Robust matching logic matched CustomTestModal
    const normalize = (s) => s.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
    const target = normalize(examName);

    // 1. Try fuzzy match against Keys
    const keys = Object.keys(METADATA);
    let metaKey = keys.find(k => normalize(k) === target);

    // 2. Special Case: GATE (examName "GATE CS" -> "GATE")
    if (!metaKey && target.startsWith('GATE')) {
        metaKey = 'GATE';

        // Return refined branch structure if possible
        if (METADATA['GATE']) {
            const branch = Object.keys(METADATA.GATE).find(b => target.includes(b));
            if (branch) {
                return METADATA.GATE[branch];
            }
            if (METADATA.GATE.CS) return METADATA.GATE.CS;
        }
    }
 ...
 */
export function getExamSubjects(examName) {
 ...
 */
export function getSubjectTopics(examName, subject) {
    const structure = getExamStructure(examName);
